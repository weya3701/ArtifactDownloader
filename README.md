...
6. 將 cache 保留在設定的 `cache` 路徑。Gradle job 若設定 `output`，會將下載套件整理成
   Maven repository layout；npm job 若設定 `output`，會將安裝完成的 `node_modules`
   保留到 `<output>/node_modules`。其他 manager 若命令自行寫入 `output`，該產物也會保留。
